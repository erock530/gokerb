...
#cgo LDFLAGS: -L/usr/local/lib/ -lk5crypto
*/
import "C"
...
func mustSign(key key, algo, usage int, data ...[]byte) []byte {
	logging.Debug("Signing with %d", algo)
...
func mustDecrypt(key key, salt []byte, algo, usage int, data []byte) []byte {
	...
	if err != nil {
		panic(fmt.Errorf("Unable to decrypt: %+v", err))
	}
	return dec
}
...
func (c *rc4hmac) SignAlgo(usage int) int {
	switch usage {
	case gssWrapSign:
		logging.Debug("Choosing to sign with signGssRc4Hmac")
		return signGssRc4Hmac
	}
	logging.Debug("Choosing to sign with unkeyed RSA-MD5")
...
func unkeyedSign(algo, usage int, data ...[]byte) ([]byte, error) {
	...
	case signMd4:
		h = md4.New()
	default:
		logging.Errorf("protocol not supported (%d)", algo)
		return nil, ErrProtocol(fmt.Sprintf("Unsupported signature algorithm: %+v", algo))
	}
...
func (c *rc4hmac) Decrypt(salt []byte, algo, usage int, data []byte) ([]byte, error) {
	...
	case gssSequenceNumber:
		if algo != cryptGssRc4Hmac && algo != cryptGssNone {
			logging.Errorf("Decrypt gssSequenceNumber, invalid algorithm: %v", algo)
			return nil, ErrProtocol(fmt.Sprintf("Decrypt gssSequenceNumber, invalid algorithm: %v", algo))
		}
	...
	case gssWrapSeal:
		...
		if algo != cryptGssRc4Hmac {
			logging.Errorf("Decrypt gssWrapSeal, invalid algorithm: %v", algo)
			return nil, ErrProtocol(fmt.Sprintf("Decrypt gssWrapSeal, invalid algorithm: %v", algo))
		}
	...
	if algo != cryptRc4Hmac || len(data) < 24 {
		logging.Errorf("Non-RC4 HMAC, not enough data: %v", data)
		return nil, ErrProtocol(fmt.Sprintf("Non-RC4 HMAC, not enough data: %v", data))
	}
	...
	if subtle.ConstantTimeCompare(chk, data[:16]) != 1 {
		logging.Errorf("Decrypt: Invalid input checksum")
		return nil, ErrProtocol(fmt.Sprintf("Decrypt: Invalid input checksum"))
	}
...
func (s *descbc) Sign(algo, usage int, data ...[]byte) ([]byte, error) {
	...
	switch algo {
	case signGssDes:
		logging.Debug("Signing with GSSDES")
		sz := 0
		for _, d := range data {
			sz += len(d)
		}
	...
	case signGssMd5Des:
		logging.Debug("Signing with GSSMD5DES")
		h = md5.New()
		for _, d := range data {
			h.Write(d)
		}
	...
	case signMd5Des:
		logging.Debug("Signing with MD5")
		h = md5.New()
	case signMd4Des:
		logging.Debug("Signing with MD4")
		h = md4.New()
	default:
		logging.Debug("Signing within unkeyed signature")
		return unkeyedSign(algo, usage, data...)
	}
...
func (s *descbc) SignAlgo(usage int) int {
	switch usage {
	case gssWrapSign:
		logging.Debug("Choosing to sign with GSSMD5DES")
		return signGssMd5Des
	}

	logging.Debug("Choosing to sign with MD5DES")
	return signMd5Des
}
...
func (s *descbc) Decrypt(salt []byte, algo, usage int, data []byte) ([]byte, error) {
	...
	case cryptDesCbcMd4:
		h = md4.New()
	default:
		logging.Errorf("descbc Decrypt: invalid algo: %v", algo)
		return nil, ErrProtocol(fmt.Sprintf("descbc Decrypt: invalid algo: %v", algo))
	}

	if (len(data) & 7) != 0 {
		logging.Errorf("descbc Decrypt: not enough data")
		return nil, ErrProtocol(fmt.Sprintf("descbc Decrypt: not enough data"))
	}
	...
	if subtle.ConstantTimeCompare(chk, data[8:8+len(chk)]) != 1 {
		logging.Errorf("descbc Decrypt: ConstantTimeCompare != 1")
		return nil, ErrProtocol(fmt.Sprintf("descbc Decrypt: ConstantTimeCompare != 1"))
	}
...
func generateKey(algo int, rand io.Reader) (key, error) {
	logging.Debug("entering generateKey with algo: %v", algo)
	switch algo {
	case cryptRc4Hmac:
		data := [16]byte{}
	...
	case cryptAES128SHA1, cryptAES256SHA1:
		k := make([]byte, 16)
		if algo == cryptAES256SHA1 {
			k = append(k, make([]byte, 16)...)
		}
	...
	logging.Warn("GenerateKey: protocol error. Algo: %v", algo)
	return nil, ErrProtocol(fmt.Sprintf("GenerateKey: protocol error. Algo: %v", algo))
}
...
func loadKey(algo int, key []byte) (key, error) {
	...
	case cryptAES128SHA1, cryptAES256SHA1:
		keyLen := len(key)
		keyCon := aesBytesToOctet(key)
		krb5key := C.krb5_keyblock{magic: 0, enctype: aesEncType(keyLen), length: C.uint(keyLen), contents: keyCon}
		return &aesHmac{key: key, algo: algo, krb5key: krb5key}, nil
	}
	logging.Errorf("loadKey: protocol not supported error. Algo: %v", algo)
	return nil, ErrProtocol(fmt.Sprintf("loadKey: protocol error. Algo: %v", algo))
}

func loadStringKey(algo int, pass, salt string) (key, error) {
	logging.Debug("loadStringKey with algo: %v", algo)
	if len(pass) == 0 {
		logging.Errorf("loadStringKey: zero length pass")
		return nil, ErrProtocol(fmt.Sprintf("loadStringKey: zero length pass"))
	}

	switch algo {
	case cryptRc4Hmac:
		if len(salt) > 0 {
			logging.Errorf("ErrProtocol - salt not supported with crytpRc4Hmac")
	...
	case cryptAES128SHA1:
		mitK5 := aesStringKey(pass, salt, 16)
		aesKey := C.GoBytes(unsafe.Pointer(mitK5.contents), C.int(16))
		return &aesHmac{key: aesKey, algo: algo,
			krb5key: C.krb5_keyblock{magic: mitK5.magic, enctype: mitK5.enctype, length: mitK5.length, contents: mitK5.contents}}, nil

	case cryptAES256SHA1:
		mitK5 := aesStringKey(pass, salt, 32)
		aesKey := C.GoBytes(unsafe.Pointer(mitK5.contents), C.int(32))
		return &aesHmac{key: aesKey, algo: algo,
			krb5key: C.krb5_keyblock{magic: mitK5.magic, enctype: mitK5.enctype, length: mitK5.length, contents: mitK5.contents}}, nil
	}
