...
func chooseAlgorithm(msg *errorMessage) (etype int, salt string, err error) {
	...
	logging.Info("No common algorithm chosen")
	return 0, "", ErrNoCommonAlgorithm
}

//DialFn is a function template for a type that can dial connection
type DialFn func(proto, realm string) (io.ReadWriteCloser, error)

//NowFn is a function template for a type that can get the current date time
type NowFn func() time.Time

//CredConfig is the configuration necessary to create connections for credentials
...
func NewCredential(user, realm, pass string, cfg *CredConfig) (*Credential, error) {
	...
	if err == nil {
		logging.Warn("ErrProtocol")
		return nil, ErrProtocol("")
	} else if !ok || rerr.ErrorCode() != int(KDC_ERR_PREAUTH_REQUIRED) {
		return nil, err
	}
...
func (c *Credential) getTgt(realm string, ctill time.Time) (*Ticket, string, error) {
	...
	logging.Debug("didnt get tgt from remote realm")

	// TGS_REQ using the local realm
	if tgt := c.lookupCache(c.tgt, c.realm, ctill, 0); tgt != nil {
		return tgt, c.realm, nil
	}
	logging.Debug("didnt get tgt from local realm")

	// Credentials created with ReadCredentialCache don't know the client key
	if c.key == nil {
		logging.Warn("Credentials created with ReadCredentialCache don't know the client key")
...
//TicketConfig is used for ticket connections
type TicketConfig struct {
	Till  time.Time
	Flags int
}

//DefaultTicketConfig are the standard ticket config options
...
func (c *Credential) GetTicket(service string, cfg *TicketConfig) (*Ticket, error) {
	...
	if err != nil {
		logging.Errorf("Unable to get ticket: %v", err)
		return nil, err
	}
	...
	for i := 0; i < 10; i++ {
		...
		logging.Warn("Unable to get the service from the TGS loop")
		return nil, ErrProtocol("Unable to get the service from the TGS loop")
	}
