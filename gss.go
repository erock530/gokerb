...
func mustDecodeGSSWrapper(data []byte) (asn1.ObjectIdentifier, []byte) {
	...
	default:
		panic(ErrProtocol("Invalid GSS Header"))
	}
...
func (t *Ticket) Connect(rw io.ReadWriter, flags int) (gssrw io.ReadWriter, err error) {
	...
	case gssAppReply:
		// continue below
	default:
		logging.Errorf("protocol not supported")
		panic(ErrProtocol(fmt.Sprintf("gsstype is not an app reply or an app error: %d", gssAppReply)))
	}
...
func (c *Credential) Accept(rw io.ReadWriter, flags int) (gssrw io.ReadWriter, user, realm string, err error) {
	...
	if err != nil {
		logging.Errorf("unable to read AP_REQ data: %v", err)
	}
	...
	if len(reqdata) < 2 {
		err := fmt.Errorf("reqdata is too short")
		panic(err)
	}
	bytes := binary.BigEndian.Uint16(reqdata[:2])
	if bytes != gssAppRequest {
		err := fmt.Errorf("reqdata is not gssAppRequest: %v - %v - %v", bytes, gssAppRequest, reqdata)
		panic(err)
	}
	...
	//Ensure that case problems with the realm and service do not stop authentication
	serviceName := principalName{
		Parts: make([]string, 0, len(tkt.Service.Parts)),
	}
	for _, s := range tkt.Service.Parts {
		serviceName.Parts = append(serviceName.Parts, strings.ToLower(s))
	}
	principal := principalName{
		Parts: make([]string, 0, len(c.principal.Parts)),
	}
	for _, s := range c.principal.Parts {
		principal.Parts = append(principal.Parts, strings.ToLower(s))
	}
	if strings.ToUpper(tkt.Realm) != strings.ToUpper(c.realm) || !nameEquals(serviceName, principal) {
		panic(ErrTicket{fmt.Sprintf("wrong service - %v - expected - %v", tkt.Service.Parts, c.principal.Parts)})
	}

	if c.kvno != 0 && c.kvno != tkt.Encrypted.KeyVersion {
		panic(ErrTicket{fmt.Sprintf("wrong key version: %d - out keytab version: %d", tkt.Encrypted.KeyVersion, c.kvno)})
	}
	...
	if !(-5*time.Minute < now.Sub(auth.Time) && now.Sub(auth.Time) < 5*time.Minute) {
		logging.Errorf("Client time is too far off of current system time: %s, %s", auth.Time.UTC(), time.Now().UTC())
	}
	...
	default:
		logging.Errorf("protocol not supported")
		panic(ErrProtocol("gss header is not no security, intagrity, or confidential"))
	}
